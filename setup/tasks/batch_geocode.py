...
class Master:
    ...
    def geo_codes_into_df(self, input_df, geo_codes_ordered_dict):
...
def run_geocode_api(n_threads, input_df, timeout_sec, queue_size=50, queue_df_size=200):
    ...
    geo_codes_orderd_dict = wrapper_api_call(df_input_addresses_queued=input_df.iloc[0:2, :])[0]
    import pdb;
    pdb.set_trace()
    col_df = master.geo_codes_into_df(geo_codes_orderd_dict)
    master_output_df = pd.DataFrame(columns=list(input_df) + list(col_df))
